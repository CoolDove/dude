...
imgui_setup_render_state :: proc(data: ^imgui.Draw_Data, state: OpenGL_State) {
    ...
    gl.VertexAttribPointer(u32(state.attrib_vtx_pos),   2, gl.FLOAT,         gl.FALSE, size_of(imgui.Draw_Vert), cast(uintptr)rawptr(offset_of(imgui.Draw_Vert, pos)));
    gl.VertexAttribPointer(u32(state.attrib_vtx_uv),    2, gl.FLOAT,         gl.FALSE, size_of(imgui.Draw_Vert), cast(uintptr)rawptr(offset_of(imgui.Draw_Vert, uv)));
    gl.VertexAttribPointer(u32(state.attrib_vtx_color), 4, gl.UNSIGNED_BYTE, gl.TRUE,  size_of(imgui.Draw_Vert), cast(uintptr)rawptr(offset_of(imgui.Draw_Vert, col)));
}
...
restore_opengl_state :: proc(state: OpenGL_Backup_State) {
    ...
    if state.last_enabled_blend       do gl.Enable(gl.BLEND);        
    else do gl.Disable(gl.BLEND);

    if state.last_enable_cull_face    do gl.Enable(gl.CULL_FACE);
    else do gl.Disable(gl.CULL_FACE);

    if state.last_enable_depth_test   do gl.Enable(gl.DEPTH_TEST)   
    else do gl.Disable(gl.DEPTH_TEST);
    if state.last_enable_scissor_test do gl.Enable(gl.SCISSOR_TEST) 
    else do gl.Disable(gl.SCISSOR_TEST);
...
compile_shader :: proc(kind: u32, shader_src: string) -> u32 {
    ...
    data := cast(cstring)cast(^u8)strings.clone_to_cstring(shader_src, context.temp_allocator);
    gl.ShaderSource(h, 1, &data, nil);
    gl.CompileShader(h);
    ok: i32;
    gl.GetShaderiv(h, gl.COMPILE_STATUS, &ok);
    if cast(bool)ok != gl.TRUE {
