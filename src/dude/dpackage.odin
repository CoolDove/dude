...
meta_pac_test :: proc() {
    ...
    file := parse_src(&psr, `
		...
		texture_white :: Texture{
			path  = "res/texture/white.png",
			scale = "res/texture/white.png",
		}
		...
		mushroom := Model {
			diffuse_tex = Texture {
				load_path = "./res/mushroom_diffuse.png",
				tint = Color { 0, 0, 1, 1 },
			},
			emission_tex = texture_white,
			scale = 1.0,
			triangulation = true,
		}
...
generate_package :: proc(file: ^ast.File) -> ^DPackage {
	...
	for stmt in file.decls {
		...
		if ok {
			values := generate_value_decl(decl)
			for v in values {
...
generate_value_decl :: proc(decl: ^ast.Value_Decl) -> []^DPacValue {
	...
	for i in 0..<count {
		...
		if decl.type != nil {
			...
			value = generate_value(value_decl)
			value.name = decl.names[i].derived_expr.(^ast.Ident).name
		}
		values[i] = value
	}
...
generate_value :: proc(expr : ^ast.Expr) -> ^DPacValue {
	value := new(DPacValue)
	#partial switch vtype in expr.derived_expr {
	case ^ast.Comp_Lit:
		comp_lit := generate_comps(expr.derived_expr.(^ast.Comp_Lit))
	...
	case ^ast.Field_Value:
		...
		v := generate_value(field_value_pair.value)
	}
	return value
}

generate_comps :: proc(complit: ^ast.Comp_Lit) -> ^DPacData {
	comp := new (DPacData)
	...
	for elem, ind in complit.elems {
		...
		case ^ast.Field_Value:
			...
			field := field_value_pair.field.derived_expr.(^ast.Ident).name
			value := generate_value(field_value_pair.value)
			value.name = field
